...
dataPromise.then(data => {
    ...
    // Retrieve the sample names from the data
...
function optionChanged(newSample) {
    // This function updates the information and charts for the new sample
    dataPromise.then(data => {
...
// This function indicates the demographic information for the selected sample
function buildMetadata(sample, data) {
    // Obtain the metadata for all samples
    var metadata = data.metadata;

    // Filter the metadata to include the selected sample
    var metadataArray = metadata.filter(sampleObj => sampleObj.id == sample);
    var selectedSample = metadataArray[0];
    var PANEL = d3.select("#sample-metadata");

    // Remove the previous demographic information
    PANEL.html("");
...
function buildCharts(sample, data) {
    ...
    // Filter the sample data to show only the selected sample
    var sampleArray = samples.filter(sampleObj => sampleObj.id == sample);
    // Filter the metadata data to display the selected sample
    var metadataArray = data.metadata.filter(sampleObj => sampleObj.id == sample);
    var selectedSample = sampleArray[0];

    // Obtain the data for the selected sample
    ...
    // Bar Chart
      ...
      // Reverse the x axis to make sure the bar chart has biggest on top down
      var barData = [{
 ...
 radius = .5;
var radians = degrees * Math.PI / 180;  // angle in radians for the needle on the gauge chart
var x = radius * Math.cos(radians); // x-coordinate for the needle
var y = radius * Math.sin(radians); // y-coordinate for the needle
...
Plotly.newPlot('gauge', gaugeData, gaugeLayout);  // Gauge chart
